...
- **🎙️ Reconnaissance de sous-titres au niveau des mots et à faible illusion avec ASR**
...
> *Le chinois utilise un modèle ASR séparé amélioré par la ponctuation, pour l'instant...
...
- ASR : Utilisez le service ASR Gemini pour la transcription
...
> **Note :** VideoLingo fonctionne avec divers services - une seule clé API pour tous les services (LLM, ASR, TTS). Ou exécutez localement avec Ollama et Edge-TTS gratuitement, sans API nécessaire !
...
1. Les performances de transcription ASR peuvent être affectées par le bruit de fond de la vidéo. Pour les vidéos avec une musique de fond forte, veuillez activer l'amélioration de la séparation vocale.
...
4. **La reconnaissance de transcription vidéo multilingue ne conservera que la langue principale**. C'est parce que les systèmes ASR utilisent généralement un modèle spécialisé pour une seule langue lors du traitement des sous-titres, et supprimeront les langues non reconnues.

5. **Impossible de doubler séparément plusieurs personnages**, car la capacité de distinction des locuteurs ASR n'est pas suffisamment fiable.
