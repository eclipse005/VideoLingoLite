...
4. **多语言视频转录识别仅仅只会保留主要语言**，这是由于 ASR 系统在处理字幕时通常使用针对单个语言的特化模型，会因为不认识另一种语言而删去。

5. **无法多角色分别配音**，ASR 说话人区分效果不够好用。
