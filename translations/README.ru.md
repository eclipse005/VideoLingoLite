...
- **🎙️ Пословное распознавание субтитров с низким уровнем искажений с помощью ASR**
...
> *Китайский пока использует отдельную модель ASR с улучшенной пунктуацией...
...
- ASR: Используйте сервис Gemini ASR для транскрипции
...
> **Примечание:** VideoLingo работает с различными сервисами - один API-ключ для всех сервисов (LLM, ASR, TTS). Или запускайте локально с Ollama и Edge-TTS бесплатно, без необходимости в API!
...
1. Производительность транскрипции ASR может быть затронута фоновым шумом видео. Для видео с громкой фоновой музыкой включите Улучшение разделения голоса.
