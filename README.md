...
### 批量处理
- **CSV 配置**：通过表格配置多个视频任务
- **YouTube 下载**：支持 YouTube 链接，自动下载最佳画质
- **自动化队列**：一键处理整个文件夹的视频
...
启动后自动打开浏览器访问 Web UI，支持上传本地视频或粘贴 YouTube 链接
...
- **三阶段分句架构**：标点符号 + 停顿切分（可选）+ LLM 长度切分
  - Stage 1 (标点)：按句子结束标点符号分句
  - Stage 1+ (停顿)：可选，按静默间隙切分
  - Stage 2 (LLM)：对超长句进行语义分析，确保字幕长度符合要求
