...
def llm_sentence_split():
    ...
    # Check if split_by_meaning.txt already exists (generated by Parakeet)
    # If so, skip LLM processing and load directly
    import os
    if os.path.exists(_3_2_SPLIT_BY_MEANING):
        with open(_3_2_SPLIT_BY_MEANING, 'r', encoding='utf-8') as f:
            sentences = [line.strip() for line in f if line.strip()]
        if sentences:
            console.print(f"[yellow]⏭️ Skipping LLM sentence splitting (loaded {len(sentences)} sentences from local ASR segments)[/yellow]")
            return sentences
