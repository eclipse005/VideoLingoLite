...
def llm_sentence_split():
    ...
    # Check if split_by_meaning.txt already exists (generated by WhisperX)
    ...
    if os.path.exists(_3_2_SPLIT_BY_MEANING):
        ...
        if sentences:
            console.print(f"[yellow]⏭️ Skipping LLM sentence splitting (loaded {len(sentences)} sentences from WhisperX segments)[/yellow]")
            return sentences
