...
# Monkey-patch torch.load to revert to pre-PyTorch 2.6 behavior.
# This is to fix a series of "Weights only load failed" errors from pyannote.audio.
...
def transcribe_audio(raw_audio_file, vocal_audio_file, start, end):
    ...
    WHISPER_LANGUAGE = load_key("asr.language")
    device = "cuda" if torch.cuda.is_available() else "cpu"
    rprint(f"🚀 Starting WhisperX using device: {device} ...")

    # Load user settings from config
    config_model = load_key("asr.model")
    config_batch_size = load_key("asr.batch_size")
    use_hotwords = load_key("asr.use_hotwords")
    hotwords = load_key("asr.hotwords")

    if device == "cuda":
        gpu_mem = torch.cuda.get_device_properties(0).total_memory / (1024**3)
        # Use configured batch size, otherwise adapt based on GPU memory
        batch_size = config_batch_size if config_batch_size else (16 if gpu_mem > 8 else 2)
        ...
        batch_size = config_batch_size if config_batch_size else 1
        compute_type = "int8"
    ...
    # Use configured model, keep Chinese special handling
    if WHISPER_LANGUAGE == 'zh':
        model_name = "Huan69/Belle-whisper-large-v3-zh-punct-fasterwhisper"
        local_model = os.path.join(MODEL_DIR, "Belle-whisper-large-v3-zh-punct-fasterwhisper")
    else:
        model_name = config_model
        # Convert model name format for local path (replace '/' with '--')
        local_model_name = model_name.replace('/', '--')
        local_model = os.path.join(MODEL_DIR, local_model_name)
    ...
    if use_hotwords and hotwords:
        ...
        rprint(f"[cyan]🔥 Using hotwords:[/cyan] {hotwords}")
