...
3. **_3_llm_sentence_split.py** - LLM-powered semantic sentence splitting
4. **_4_1_summarize.py** - Content summarization and terminology extraction
5. **_4_2_translate.py** - Two-stage translation (literal → free translation)
6. **_5_split_sub.py** - Subtitle splitting based on length constraints
7. **_6_gen_sub.py** - Final subtitle generation (SRT format)

### Key Design Patterns

**difflib Alignment Algorithm** (Core Innovation):
- Located in `core/_3_llm_sentence_split.py` and `core/_6_gen_sub.py`
...
st.py (entry point)
  ...
  │    └─> models.py         # File path definitions
...
- Critical in `_3_llm_sentence_split.py` (sentence boundary detection) and `_5_split_sub.py` (alignment validation)
